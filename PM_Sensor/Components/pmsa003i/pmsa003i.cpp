...
esp_err_t PMSA003I::read_bytes(uint8_t *buffer, size_t length) {
    ...
    i2c_master_write_byte(cmd, (PMSA003I_ADDRESS << 1) | I2C_MASTER_WRITE, true);
    i2c_master_write_byte(cmd, 0x00, true); // Send register address (if required)
    i2c_master_start(cmd);
...
void PMSA003I::parse_data_(uint8_t *buffer) {
    // Assuming buffer contains PM2.5, and PM10 data at specific indices
    pm2_5_ = (buffer[6] << 8) | buffer[7];  
    pm10_0_ = (buffer[8] << 8) | buffer[9];  

    ESP_LOGI(TAG, "PM2.5: %.1f µg/m³, PM10: %.1f µg/m³", pm2_5_, pm10_0_);
}
